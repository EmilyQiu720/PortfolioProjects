...
# Function to create a folium map with labeled origins and destinations for two riders and display the chosen route
def create_route_map(origin_i, dest_i, origin_j, dest_j, trip_order):
    ...
    folium.Marker(location=origin_i, icon=folium.DivIcon(html=f"<div style='{text_box_style}'><b>Rider i Pickup</b></div>")).add_to(map_folium)

    folium.Marker(location=dest_i, icon=folium.Icon(color=rider_i_color, icon=dropoff_icon)).add_to(map_folium)
    folium.Marker(location=dest_i, icon=folium.DivIcon(html=f"<div style='{text_box_style}'><b>Rider i Dropoff</b></div>")).add_to(map_folium)
    ...
    folium.Marker(location=origin_j, icon=folium.DivIcon(html=f"<div style='{text_box_style}'><b>Rider j Pickup</b></div>")).add_to(map_folium)

    folium.Marker(location=dest_j, icon=folium.Icon(color=rider_j_color, icon=dropoff_icon)).add_to(map_folium)
    folium.Marker(location=dest_j, icon=folium.DivIcon(html=f"<div style='{text_box_style}'><b>Rider j Dropoff</b></div>")).add_to(map_folium)
...
# Function to export the notebook to a Python script
def export_notebook(teamname):
    """
    Exports the current notebook (.ipynb) to a Python script (.py), renaming it based on the team name,
    and removes all print statements from the exported Python script.
    
    Parameters:
        teamname (str): The name to include in the exported .py file's name.
    """
    # Hardcoded notebook name for now (replace with dynamic detection if needed)
    notebook_name = "student_policies.ipynb"
    ...
    try:
        ...
        renamed_script_name = f"{teamname}_Policies.py"
        ...
        # Remove all print statements from the renamed script
        with open(renamed_script_name, 'r') as file:
            lines = file.readlines()

        # Filter out lines containing print statements
        filtered_lines = []
        for line in lines:
            # Remove lines with standalone print statements
            if not re.match(r'^\s*print\(.*\)\s*$', line.strip()):
                filtered_lines.append(line)

        # Write back the modified script without print statements
        with open(renamed_script_name, 'w') as file:
            file.writelines(filtered_lines)

        print(f"Converted {notebook_name} to {renamed_script_name} successfully, and removed print statements!")
